...
Also verify Phase 8 gate status before training: Phase 9 training must use the documented Decision A/B/C state and the available Phase 8D history/metric artifacts. The local non-GPU checkout does not need `.pt` checkpoint files, because they may remain on the Windows GPU/training machine. If Decision B is selected, use `unetpp_student` only when the training-machine run produced checkpoints there and copied/summarized histories show acceptable Dice/IoU; otherwise use `classical_improved` or revise the decision before training.
