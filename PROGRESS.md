...
### Phase 15 progress
- Added **Phase 15 - Ablation Studies** to `main.ipynb`. The new cell is definition-only by default (`PHASE15_RUN_ABLATIONS = False`) and writes its protocol/configuration audit to `outputs/phase15_ablations/` when run.
- Implemented 21 named configurations from `PLAN.md`: full-model confirmations, SimCLR, attention, layer-identity, tabular, mask, biomarker, prototype, uncertainty, calibration, label-smoothing, class-balanced-sampling, mask-source, single-layer, two-layer, image-only, images-plus-tabular, and images-plus-tabular-plus-masks comparisons.
- Added a configurable Phase 15 model wrapper so ablations do not mutate or require rerunning completed Phase 9-12 cells. It supports arbitrary Sup/Deep/CC subsets, attention-versus-direct token aggregation, optional identity embeddings, optional tabular and mask branches, and a true standard-linear alternative to the prototype head.
- Reused Phase 12's leakage-safe outer/inner patient splits, Stage 2 frozen-backbone then Stage 3 fine-tuning schedule, checkpointing/resume behavior, AMP, gradient clipping, and inner-validation macro-F1 early stopping. Each configuration/fold is freshly seeded and writes to an isolated output directory.
- Added explicit mask-source handling: `unetpp_student` is the default operational source and uses Phase 9 materialized masks; `classical_improved` is an explicit pseudo-mask comparator only. Every prediction and fold summary records the effective source and whether generated biomarkers were included.
- Added independent per-fold outer-test OOF prediction exports, optional 30-pass MC-dropout prediction aggregation, validation-only temperature fitting, a consolidated five-fold mean ± standard-deviation report for macro-F1 and balanced accuracy, and a marginal-contribution table against the full-model OOF result.
- Expanded Phase 15 observability and restart safety: each epoch continues to persist Phase 12 train, inner-validation, and outer-test loss, macro-F1, balanced accuracy, and per-class recall; Phase 15 now additionally writes the selected best-epoch train/validation/test row, full OOF eye- and patient-level Phase 14 metric bundles, per-class metrics, confusion matrices, reliability artifacts, and MC-dropout entropy/classwise epistemic-variance uncertainty summaries for every ablation where MC dropout is enabled.
- Added configuration-level and full-experiment console progress with elapsed time, active-fold duration, rolling total ETA, current/total fold-run counts, per-configuration final accuracy/macro-F1, and `outputs/phase15_ablations/phase15_run_state.json` after every fold. A backward-compatible optional Phase 12 per-epoch callback now refreshes this full-experiment ETA during the first active fold as well. Completed fold summaries are skipped safely on restart only when their stored configuration exactly matches; interrupted Stage 2/3 runs continue through the existing checkpoint resume mechanism.
- Hardened Phase 15 for partial notebook reruns: if the in-memory Phase 12 training function predates the optional progress-callback signature, Phase 15 now falls back to the existing resumable trainer and stage-level epoch ETA instead of failing. Added a Phase 15-owned weighted sampler with a writable NumPy buffer to avoid the current Windows PyTorch non-writable-array warning.
- Validated the edited notebook as JSON and compiled the extracted Phase 15 cell successfully. It was not executed against data or GPU checkpoints in this local review checkout.

### Phase 13 tweaked progress
- Added a separate **Phase 13 Tweaked - Explainability for Correct Predictions Only** cell to `main.ipynb`. It leaves the original Phase 13 outputs unchanged and writes a separate bundle under `outputs/phase13_tweaked_correct_predictions/`. In a fresh kernel it safely loads only the original Phase 13 definitions, without rerunning its artifact generation.
- The tweaked cell applies the temperature-calibrated `pred == label` gate before every per-sample artifact is generated: Grad-CAM overlays and heatmaps, attention-dominance rows and aggregates, tabular attribution CSVs and waterfall plots, plus all integrated and cross-validation summaries.
- The cell retains Phase 13's folds, deterministic selection, class balancing, Grad-CAM targets, SHAP policy, batching, and logging controls. Its fold summaries separately report evaluated, saved-correct, and skipped-incorrect sample counts.

## 2026-07-12

### Documentation light-theme hardening
- Applied a shared final light-theme contract to all current HTML explainers in `docs/`.
